...
// @downloadURL  https://raw.githubusercontent.com/rwil02/herowars/main/HeroWarsHelper.user.js
// ==/UserScript==
...
(function () {
    ...
    let hw_Settings = JSON.parse(GM_getValue("hw_Settings", "{}"));
    ...
    function setupRecommendationsHeaders(recommendationType, results) {
        ...
        for (let k = 0; k < results.length; k++) {
            const opponent = results[k];
            let winStyle = 'never';
            if (opponent.battles.length) {
                winStyle = 'win0';
                if (opponent.winPercent >= 0.05) {
                    winStyle = "win1";
                    if (opponent.winPercent >= 0.5) {
                        winStyle = "win2";
                        if (opponent.winPercent >= 0.75) {
                            winStyle = "win3";
                            if (opponent.winPercent >= 0.85) {
                                winStyle = "win4";
                            }
                        }
                    }
        ...
        XMLHttpRequest.prototype.send = function (data) {
            try {
                ...
                const callsToIgnore = [
                    ...
                    "bundleGetAllAvailableId", "campaignStoryGetList", "clanGetActivityStat", "chatsGetAll", "chatGetInfo",
                    ...
                    "clanRaid_ratingInfo", "clanRaid_farmBossReward", "clanRaid_usersInBossBattle", "clanRaid_endBossBattle", "clanRaid_startBossBattle",
                    "clanRaidSubscription_getInfo", "clanWarGetBriefInfo", "clanWarGetWarlordInfo",
                    "consumableUseLootBox", "coopBundle_getInfo", "crossClanWar_getBriefInfo", "crossClanWar_getSettings", "dailyBonusGetInfo",
                    "dungeonEndBattle", "dungeonStartBattle", "epicBrawl_getBriefInfo",
                    ...
                    "heroTalent_farmReward", "heroTalent_getReward", 
